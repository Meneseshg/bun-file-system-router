...
#include <set>
#include <algorithm>
#include <atomic>       // Para contador de hilos seguro
#include <sys/stat.h> // Para crear carpetas
#include <cstdio>     // Para remove y rename

#include <dirent.h>   // Para leer directorios

// --- Configuración Global de Estado ---
std::atomic<int> g_active_processing_tasks(0); // Contador de hilos de procesamiento activos
...
// Hilo de Procesamiento de Video (Worker)
// Se encarga de unir los segmentos y notificar, sin bloquear la cámara.
void process_video_job(std::vector<std::string> segments, std::string output_path, std::string curl_cmd)
{
    g_active_processing_tasks++;
    std::cout << "[WORKER] Iniciando procesamiento de video. Tareas activas: " << g_active_processing_tasks.load() << std::endl;

    // 1. Crear archivo de lista para concat
    // Usamos un nombre único para la lista basado en el PID del hilo o un hash simple para evitar colisiones
    // Aunque como cada proceso es independiente, podemos usar el path del video como base única.
    std::string list_path = "/dev/shm/concat_" + output_path.substr(output_path.find_last_of('/') + 1) + ".txt";
    
    std::ofstream listFile(list_path);
    for (const auto &s : segments)
        listFile << "file '" << s << "'\n";
    listFile.close();

    // 2. Ejecutar FFmpeg
    std::string concat_cmd = "ffmpeg -y -f concat -safe 0 -i " + list_path + " -c copy -an -movflags +faststart " + output_path;
    // Redirigimos stderr a null para no ensuciar el log principal, o lo dejamos si se quiere debug
    // concat_cmd += " > /dev/null 2>&1"; 
    std::system(concat_cmd.c_str());

    // 3. Limpiar lista temporal
    std::remove(list_path.c_str());

    // 4. Notificar vía CURL (si existe)
    if (!curl_cmd.empty())
    {
        std::string final_curl = curl_cmd;
        replace_all(final_curl, "{FILE_NAME}", output_path);
        std::system(final_curl.c_str()); // Ejecutamos sincrono AQUÍ, porque ya estamos en un hilo aparte
    }

    g_active_processing_tasks--;
    std::cout << "[WORKER] Video procesado: " << output_path << ". Tareas restantes: " << g_active_processing_tasks.load() << std::endl;
}
...
int main()
{
    ...
    while (true)
    {
        ...
        if (sensor == 1)
        {
            ...
            // Si es el inicio de la grabación, filtramos para tener solo los últimos 5 segmentos de pre-roll
            // Si el usuario reporta 20s con 10 segmentos, significa que cada segmento dura ~2s.
            // Con 5 segmentos, deberíamos obtener ~10s de historial.
            if (captured_segments.empty())
            {
                ...
                if (now_segs.size() > 5)
                {
                    ...
                    std::vector<std::string> recent_segs(now_segs.end() - 5, now_segs.end());
        ...
        if (waiting_post_record)
        {
            ...
            else
            {
                ...
                // LANZAR HILO DE PROCESAMIENTO (Fire & Forget)
                // Pasamos una copia de captured_segments, path y curl_template
                std::thread t_worker(process_video_job, captured_segments, current_video_path, curl_template);
                t_worker.detach();
